...
figma.on("run", ({ parameters }) => {
    ...
    if (parameters) {
        ...
        let images = new Array(), nonimages = new Array();
        frames.map((e) => {
            e.fills.map((a) => {
                a.type;
        ...
        ghostifyText(text);
        const nest = () => {
            const nest = Array.from(figma.currentPage.findAll((item) => item.parent.type === "PAGE"));
            const topframes = Array.from(figma.currentPage.findAll((frame) => frame.type === "FRAME" && frame.parent.type === "PAGE"));
                ...
                nest.map((a) => {
                    topframes.map((e) => {
                        if (!0 === overlap(a, e) && a.id != e.id) {
                            let t = Math.abs(e.x - a.x), h = Math.abs(e.y - a.y);
                            e.appendChild(a), (a.x = t), (a.y = h);
                        }
                    });
                });
            function overlap(e, h) {
                return (e.parent.children.indexOf(e) > h.parent.children.indexOf(h) &&
                    e.x >= h.x &&
